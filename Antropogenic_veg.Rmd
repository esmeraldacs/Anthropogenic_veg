...
plot0 <- ggplot2::ggplot() + 
  ...
  geom_sf(data = maps1, aes(fill=Value, colour=Value)) +
...
pmp_meta_sf <- pmp_meta_sf %>%
  sf::st_join(maps, join = sf::st_within)  # Join based on spatial location
...
ggplot2::ggplot() + 
  ...
  geom_point(data=pmp_meta_zone, aes(x=longitude, y=latitude, fill=Value, colour=Value)) +
  jcolors::scale_color_jcolors(palette = "pal9") +
  jcolors::scale_fill_jcolors(palette = "pal9") +
...
raster::plot(footprint_raster)
```



```{r}
# Ensure that the CRS of maps1 matches the CRS of the raster
maps1 <- st_transform(maps1, crs = st_crs(footprint_raster))

# Extract cell numbers (indices) within each polygon
cell_indices <- extract(footprint_raster, maps1, cellnumbers = TRUE)
...
extract_pixel_info <- function(cell_indices, polygon_value) {
  ...
  # Return data with the polygon's Value, but filter out NAs
  data <- data.frame(lon = pixel_coords[, 1], lat = pixel_coords[, 2], value = pixel_values, Value = polygon_value)
  return(data[!is.na(data$value), ])  # Filter out rows where value is NA
}

# Apply the function to all polygons and include the polygon Value in each case
pixel_data <- mapply(extract_pixel_info, cell_indices, maps1$Value, SIMPLIFY = FALSE)

# Split the results by 'Value' to prevent a massive dataframe
# This will create a list of dataframes, each corresponding to a different 'Value'
pixel_data_split <- split(do.call(rbind, pixel_data), do.call(c, lapply(pixel_data, function(x) x$Value)))
```



```{r}
saveRDS(pixel_data_split, "Output_data/pixel_data_split.rds")
```



```{r}
pixel_data_split <- readRDS("Output_data/pixel_data_split.rds")
```
...
# Create a data frame for each Value from the pixel_data_split list
# Combine all the dataframes into one (with lat, lon, value, and Value)
...
# Plot using ggplot2
plot1 <- ggplot() +
  geom_sf(data = latam_polygon, fill = NA, color = "black") +  # Plot the polygon
  ...
  facet_wrap(~ Value) +  # Create a facet for each 'Value'
  theme_minimal() + 
  theme(legend.position = "bottom") +  # Adjust the legend position
  labs(title = "Raster Values by Value", fill = "Raster Value")  # Add titles and labels
...
```{r}
ggsave(plot = plot1, "plot_zones.jpg", units = "cm", width = 23, height=18)
```



```{r}
# Combine all the data back into a single dataframe (after splitting by Value)
...
# Plot histogram of the 'value' column for each 'Value' as a separate facet
plot2 <- ggplot(pixel_df, aes(x = value)) +
  geom_histogram(binwidth = 1, color = "black", fill = "skyblue", alpha = 0.7) +  # You can adjust binwidth as needed
  facet_wrap(~ Value, scales = "free_y") +  # Create a facet for each 'Value' with independent y-scales
  theme_minimal() +
  labs(title = "Histogram of 'value' within each 'Zone'", x = "Raster Value", y = "Frequency") +
...
```{r}
ggsave(plot = plot2, "plot_histogram.jpg", units = "cm", width = 20, height=11)
```


```{r}
# Combine all the data back into a single dataframe
pixel_df <- do.call(rbind, pixel_data_split)
...
plot3 <- ggplot(pixel_df, aes(x = value)) +
  geom_histogram(binwidth = 1, color = "black", fill = "skyblue", alpha = 0.7) +  # Adjust binwidth as needed
  facet_wrap(~ Value, scales = "free_y") +  # Create a facet for each 'Value'
  geom_vline(data = median_values, aes(xintercept = median_value), color = "red", linetype = "dashed", size = 1) +  # Add vertical line at median
  theme_minimal() +
  labs(title = "Histogram of 'value' within each 'Value'", x = "Raster Value", y = "Frequency") +
  theme(strip.text = element_text(size = 10))  # Adjust facet label size if needed
...
```{r}
ggsave(plot = plot3, "plot_histogram_median.jpg", units = "cm", width = 20, height=11)
```
...
medians_df <- data.frame(Value = names(medians_per_value), Median_Value = unlist(medians_per_value))
...
pmp_usage_meta <- pmp_meta_zone |>
  ...
  mutate(class_exact = case_when(human_footprint_exact < median_human_footprint ~ "low", TRUE ~ "high"),
         class_12km = case_when(human_footprint_avg12km < median_human_footprint ~ "low", TRUE ~ "high"))
...
```{r}
saveRDS(pmp_usage_meta, "Output_data/pmp_usage_meta.rds")
```
...
pmp_usage1 <- pmp_usage_meta |>
  dplyr::select(ID, Value, class_12km) |>
...
pmp_usage_split <- split(pmp_usage1, pmp_usage1$Value)
...
n_samples_train <- pmp_usage_split %>%
  map(~ .x %>%
        dplyr::select(Value, class_12km) %>%
        group_by(class_12km) %>%
        add_tally() %>%
        dplyr::select(Value, class_12km, n) %>%
        distinct() %>%
        ungroup()) %>%
  bind_rows()
...
```{r}
ggsave(plot = plot4, "Output_figures/n_samples_train.jpg", units = "cm", width = 10, height = 6)
```
...
# Access a specific transformed dataframe, e.g., for Value "13"
...
```{r}
# Apply the task to each dataframe in pmp_usage_sampled and pmp_usage_split for each Value
test_data_list <- map2(pmp_usage_sampled, pmp_usage_split, ~ {
  
  # Get the unique Value from the sampled data
  zona <- unique(.x$Value)
  
  # Perform the anti_join and transformation for each value
  .y %>%
    anti_join(.x, by = c("ID", "Value", "class_12km")) %>%
    ungroup() %>%
    pivot_longer(cols = -c(ID, Value, class_12km)) %>%
    filter(value > 0) %>%
    mutate(percent = value * 100) %>%
    dplyr::select(-value) %>%
    mutate(zone = zona)  # Add the zone from the sampled data
})
...
```{r}
saveRDS(train_data_list,"Output_train_test/train_data_run1.rds")

saveRDS(test_data_list,"Output_train_test/test_data_run1.rds")
```
...
```{r}
train_data_list <- readRDS("Output_train_test/train_data_run1.rds")
